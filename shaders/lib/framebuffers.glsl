...
uniform sampler2D colortex4; // Shading information buffer (vanillaAO*textureAO, ?, ?, ?)
uniform sampler2D colortex5; // Previous SSRT buffer, clearing disabled
uniform sampler2D colortex6; // Previous buffer values (depth, ?, ?, ?), clearing disabled
...
{
    return texture2D(colortex6, coord);
}
